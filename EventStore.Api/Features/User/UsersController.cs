...
namespace EventStore.Api.Features.User
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        /// <returns></returns>
        [HttpPost("register")]
        public async Task<Response> Register([FromBody] Request request)
        {
            var scope = _mediatorFactory.CreateScope();
            var mediatorResponse = await scope.SendAsync(RegisterUserMediatorCommand.CreateCommand(request.UserName, request.FirstName, request.LastName, request.Password), default);
